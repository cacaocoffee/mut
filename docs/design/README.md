...
[ADR-0007](../decisions/ADR-0007-design-magazine.md), 이전 시스템의 채택 경위는
...
- **구분선을 여백으로 바꾸지 않는다.**
- **사진은 `.grayscale` 래퍼를 통과시킨다.** 이미지를 착색하지 않는다.
...
> 완화는 실측표 갱신과 함께 별도 결정으로 한다 (ADR-0007 결정 1).
...
| `.hr` | 규칙선 |
| `.grayscale` | 이미지 래퍼 — 모든 콘텐츠 사진이 통과한다 |
