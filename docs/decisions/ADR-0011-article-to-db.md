...
6. **이미지 업로드** — 편집기에서 사진을 골라 올리면 GCS 버킷에 담고 공개 URL 을 `hero`·
   figure 블록 `src` 에 채운다. 엔드포인트는 `POST /api/v1/admin/media`(multipart, editor 이상).
   저장소를 R2 대신 GCS 로 둔 근거는 ADR-0007 개정(2026-08-27)에 있다 — API 와 같은 GCP
   프로젝트라 서비스 계정이 ADC 로 붙어 키가 없다. 미디어 테이블은 아직 없어 URL 만 돌려준다.
