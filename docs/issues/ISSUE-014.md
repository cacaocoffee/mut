...
25. `회수시_CocktailUnpublished_이벤트가_발행된다` — 색인에서 내려야 한다
26. `보관시_CocktailArchived_이벤트가_발행된다`
27. `이름_별칭_변경시_색인_갱신_이벤트가_발행된다` — 발행 상태 그대로여도 색인은 바뀌어야 한다
28. `cocktail_모듈이_search를_참조하지_않는다` — 경계 테스트 (이슈 001)

### 트랜잭션

29. `감사_기록이_상태_전이와_같은_트랜잭션이다` — 전이가 롤백되면 감사도 없다
30. `감사_기록_실패가_전이를_막는다` — 감사 없는 발행은 없다 (`PRIN-T08`)
