...
6. `중복_북마크는_거부되거나_멱등이다` **결정** — UNIQUE 제약. **멱등 200**
...
21. `잘못된_토큰은_404`
22. `공유_응답에_소유자_개인정보가_없다` — 표시명 정도 **결정**
23. `공유_응답에_내부_id가_없다`
24. `공유_토큰을_재발급하거나_해제할_수_있는가` **결정** — SPEC에 없다. **제공 안 함**
25. `공유된_컬렉션에_미발행_항목이_있으면_제외된다`

### 참조 무결성 (SPEC-06 §3.5 — 앱 책임)

26. `대상이_archived되면_북마크가_어떻게_되는가` **결정** — FK가 없어 dangling이 생긴다. **조회 시 필터링**(행은 유지)
...
31. `응답에_내부_id가_없다` **결정** — `DELETE /me/bookmarks/{id}`는 id를 쓴다(SPEC-07 §2.5). **본인 리소스라 허용**
32. `캐시_헤더가_없다` — 개인 데이터
...
**하지 말 것**:
- OG 태그 — 이슈 044
- 북마크 UI — Phase 1a 화면 목록에 명시 없음 **결정** (SCREENS-04 마이·저장이 미작성 — G-09). **GAPS 등재**
- 내 술장 — Phase 2
