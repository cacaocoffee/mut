...
20. `탈퇴한_행위자의_표시가_구분된다` ⚖️ — 이름을 못 가져온다. **보수적으로 "탈퇴한 사용자" 표기** + GAPS
...
29. `응답에_불필요한_개인정보가_없다` ⚖️ — `before`/`after` JSONB에 뭐가 담기나. 콘텐츠 필드라 개인정보는 아니지만 **에디터 이름 정도**는 필요. 보수적으로 **행위자 표시명만** + GAPS
...
⚖️ **보수적 판단**: Phase 1a 규모(발행 500건 수준)에서는 풀스캔도 견딘다. **인덱스를 추가하지 않는다** — SPEC-06 §5에 없는 인덱스를 임의로 늘리지 않는다. 느려지면 GAPS 등재 후 추가.
...
- [ ] ⚖️ 2건(탈퇴 표기·before/after 개인정보) `GAPS.md` 등재
