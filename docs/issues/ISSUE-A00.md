...
### 7. `NFR-L-05` 분해 — "법률 검토 1회"가 과대 포장돼 있다 (사용자 결정, 2026-08-07)

SPEC-04 `NFR-L-05`는 **"법률 검토 1회 완료 → 정식 오픈 차단"** 하나로 묶여 있다.
[SPEC-08 §8](../spec/SPEC-08_보안_권한_개인정보.md)의 검토 항목 3건을 뜯어보면 **성격이 다르다.**

| 항목 | 성격 | 언제 |
|---|---|---|
| ① 전면 성인 인증이 실제로 요구되는가 | **변호사 자문** | 접점이 약하다 — [ADR-0004](../decisions/ADR-0004-age-gate.md)의 분석대로 판매를 안 한다 |
| ② 브랜디드 콘텐츠 주류광고 기준 | **변호사 자문** | **유료 브랜드 노출을 시작할 때.** 그전엔 접점 0 (이슈 008·023의 `is_sponsored` 가드) |
| ③ `audit_log` 파기 예외 적법성 | **문서 작성** | **실사용자 수집 전.** 변호사가 아니라 처방침 문안의 일 |

③은 소셜 로그인(`FR-USER-001`, 1a P0)이 붙는 순간 **개인정보 처리방침이 법정 의무**가 되는 것과 한 덩어리다.
수집 항목이 적고(SPEC-08 §5.1이 생년월일·전화번호·주소·결제를 이미 배제) SPEC-08 §5.3 자신이 **"이 예외는 개인정보 처리방침에 명시한다"** 고 적어 뒀다.

**개정안**

```
삭제:  NFR-L-05  법률 검토 1회        → 정식 오픈 차단
신설:  NFR-L-05  ①② 변호사 자문       → 유료 브랜드 노출 시작 차단   (트리거 기반)
개정:  NFR-L-04  처방침·약관 문안 존재 → 실사용자 수집 차단 (③ 포함)  (기존 "페이지 존재"를 문안까지로)
```

**효과**: 1a는 물론 **오픈까지 변호사 없이 간다.** 유료 제휴 시작 시점에만 한 번 — 그때는 계약서도 필요하다.

⚖️ **이 판단도 구조적 읽기다** — ADR-0004가 스스로 밝힌 것과 같은 한계다. 개정 시 그 문장을 함께 남긴다.

### 8. `batch_run` 재검토 (G-21)
...
**결정** `NFR-D-01`이 "배포 차단"이라 배치 실행 여부를 알아야 한다는 논리는 성립한다. **판단 필요.**
...
7. **`NFR-L-05`가 트리거 기반으로 분해됐다** — "법률 검토 1회 = 정식 오픈 차단" 문장이 없다
8. `batch_run` 이 근거를 얻었거나 이슈 016에서 빠졌다
9. `profile` 5축 컬럼이 SPEC-06에 추가됐다 (이슈 036 — 데이터를 잃으면 다시 못 만든다)

## DoD

- [ ] 위 9항 전부
- [ ] `GAPS.md` G-18~G-21에 **해소 표시** + 무엇을 어떻게 고쳤는지
- [ ] **4번·8번은 제품 결정** — 결정 없이 닫지 않는다
- [ ] **7번(`NFR-L-05` 분해)에 "구조적 읽기"라는 한계 문장을 함께 남긴다** (ADR-0004 방식)
- [ ] 영향받는 이슈(016·022·026·031·032·036·039·041·044·045)의 **결정** 항목 정리
- [ ] 커밋: `docs(spec): SPEC-06 보강·문서 충돌 해소·NFR-L-05 분해 (G-18~G-21)`
