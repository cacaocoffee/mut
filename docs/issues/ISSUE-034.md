...
17. `저장_실패가_202를_막지_않는다` **결정** — DB 장애 시. **202 유지 + 로그** (사용자 흐름 우선)
...
22. `occurredAt이_필수다`
23. `path에_쿼리스트링이_없다` — **제거하거나 거부** **결정** **서버에서 절삭**
...
29. `payload가_알려진_필드만_받는다` **결정** — 임의 필드 허용 시 개인정보가 샌다. **event_type별 스키마 검증**
