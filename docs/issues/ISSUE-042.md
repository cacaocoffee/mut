...
6. `초성으로_검색된다` — `ㅁㄹㄱㄹㅌ`
7. `초성_프리픽스가_검색된다`
8. `초성_입력이_UI에서_안내된다` ⚖️ — 사용자가 초성 검색 가능함을 아는가. **보수적으로 플레이스홀더에 힌트** + GAPS
...
13. `그룹_순서가_결정론적이다` ⚖️ — 칵테일 우선 (weight). + GAPS
...
18. `0건_안내가_다음_행동을_제시한다` ⚖️ — 탐색으로 유도 등 + GAPS
...
- [ ] ⚖️ 4건(초성 힌트·그룹 순서·0건 안내·빈 그룹) `GAPS.md` 등재
