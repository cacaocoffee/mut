...
4. `dash가_환산되는가` ⚖️ — `FR-COCKTAIL-019`는 "**고정 표기**(`1조각`·`2 dash`)는 배수에서 제외"라 한다. **`2 dash`가 `amount_label`인지 `amount+unit`인지** 모호하다.
   → **`amount_label`이 있으면 제외, `unit=dash`는 환산**. 시드(이슈 036)가 `2 dash`를 어디에 넣었는지에 달렸다 + **GAPS 등재**
...
9. `ml에서_oz로_토글된다`
10. `변환_비율이_일관된다` — 1 oz = 29.5735ml ⚖️ (30ml 반올림?)
11. `dash는_변환되지_않는다`
12. `barspoon은_변환되지_않는다`
13. `piece는_변환되지_않는다`
14. `top_up도_변환되지_않는다`
15. `oz_표기가_읽기_쉽다` — 1.0141 oz 같은 값 처리 ⚖️ (분수 표기?)
16. `토글_상태가_유지된다` **결정** — 세션·로컬 저장
...
**결정** 다만 이슈 010의 `isScalable()`이 **응답 DTO에 포함되는지** 이슈 020에서 확인해야 한다. 없으면 020에 추가 요청 (CONVENTIONS §4 — `owns:` 밖) + GAPS.
...
**결정** **바텐딩 관례로는 1 oz ≈ 30ml**로 반올림하는 경우가 많다. 정확한 변환은 `1.0141 oz` 같은 값을 만든다.
**정확한 비율 + 소수 1자리 반올림**. 표기 방식(분수 vs 소수)은 GAPS 등재 — SCREENS-01 확인 필요.
...
- 맛 프로필 레이더 (`FR-COCKTAIL-023`) — **P1** (데이터는 이슈 036의 `profile` **결정**)
- 서버 왕복 (RED 35)
