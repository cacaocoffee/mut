...
11. `초성_섞인_입력을_처리한다` **결정** — `마ㄹㄱ`. **일반 검색으로 처리**
...
18. `빈_그룹은_생략되는가` **결정** — **빈 배열로 포함** (클라이언트가 자리를 알 수 있게)
...
**결정** SPEC-05 §6은 "별칭은 `aliases[]`로 관리하고 어드민에서 편집한다"고 했다 — **띄어쓰기 변형을 에디터가 손으로 넣으라는 뜻**일 수도 있다. **양쪽 다**: 정규화 매칭 + 에디터 별칭. GAPS 등재.
