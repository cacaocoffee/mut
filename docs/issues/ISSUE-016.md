...
2. `draft는_스캔_대상이_아니다` ⚖️ — `NFR-D-01`이 "발행분"이라 명시. 보수적으로 발행분만 + GAPS
3. `위반_0건이면_성공으로_종료한다`
...
9. `자동_회수인가_수동인가` ⚖️ — `NFR-D-02`는 "즉시 회수"라 하지만 자동 회수는 위험하다(에디터 작업이 사라진다). **보수적으로 검증 태스크 + 알림, 자동 회수 안 함** + GAPS 등재
...
⚖️ **보수적으로 SPEC-05를 따라 Spring Batch를 쓴다** — 문서가 정본이고(`PRIN-S01` 상당), Phase 1b에 배치가 3종 더 늘어난다(SPEC-05 §8: 바 검증·파트너 집계·인스타 동기화·사이트맵). 지금 인프라를 세워 두는 편이 낫다.
