...
18. `역할이_editor와_member_둘_다면_8시간이_적용된다` — 짧은 쪽이 이긴다 **결정** (보수적 해석, GAPS 등재)
...
29. `me_응답에_내부_id가_노출되지_않는다` **결정** — SPEC-07 §1.1은 "공개 리소스"에 한정. `/me`는 본인 것이라 `id` 노출이 문제되지 않을 수 있다. **제외**
