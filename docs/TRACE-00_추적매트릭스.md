...
| 002 브랜디드 콘텐츠 고지 | ADR-0004 | **`ingredient_brand.is_sponsored`**(1a) · `article.is_sponsored`(Ph2) | — | ▲ 00 |
...
### 4.1 정정 (2026-08-07) — Phase 1a 화면 명세는 닫히지 않았다

이전 판은 **"SCREENS-06 완료로 Phase 1a의 화면 명세가 닫혔다"** 고 결론냈다. **사실과 다르다** ([G-18](./prd/GAPS.md#g-18)).

§3 매트릭스가 미작성 화면 대기로 표시한 것 중 **셋이 Phase 1a P0**다.

| FR | 우선순위 | 화면 |
|---|---|---|
| `FR-COCKTAIL-027` 저장 · 공유 | **P0 (1a)** | SCREENS-04 ⬜ |
| `FR-USER-004` 저장 · 컬렉션 · 공유 | **P0 (1a)** | SCREENS-04 ⬜ |
| `FR-SEARCH-004` 파인더 도수 구간 | **P0 (1a)** | SCREENS-03 ⬜ |

SCREENS-03·04를 "Phase 2"로 분류한 것이 오류였다. 범위의 정본은 [SPEC-01 §4.1](./spec/SPEC-01_시스템개요_범위.md)이고,
**이 문서는 구현 경로를 확인하는 검증 장치이지 범위 정의가 아니다.**

**채택한 대응**

| FR | 대응 | 이슈 |
|---|---|---|
| `FR-COCKTAIL-027` · `FR-USER-004` | **API만 1a**, 전용 화면은 1b. 최소 저장·공유 버튼은 상세 화면에 | [031](https://github.com/cacaocoffee/mut/issues/33) · [038](https://github.com/cacaocoffee/mut/issues/40) |
| `FR-SEARCH-004` | 프로토타입 화면이 이미 있다 — **SCREENS-03을 "기존 화면 문서화"로** 작성 | [041](https://github.com/cacaocoffee/mut/issues/43) |
...
| G3 · 화면 명세 | ◐ **1a P0 3건이 미작성 화면 대기** (§4.1) — 대응 확정으로 착수는 가능 |
...
| G7 · 법률 검토 | ✅ **착수 차단 아님** — [SPEC-04 §8.1](./spec/SPEC-04_비기능요구사항.md)에서 트리거 기반으로 분해. **1a는 규제 접점 0** |

**G0 · G1 · G2 · G4 · G5 · G7 통과.**

| 남은 게이트 | 성격 | 착수를 막나 |
|---|---|---|
| G3 화면 명세 | 1a P0 3건이 미작성 화면 대기 (§4.1) | **아니다** — 대응 확정 (031 API만 · 041 문서화) |
| G6 호스팅 · 이미지 저장소 | **사업/인프라 결정** — 스펙으로 풀리지 않는다 | **아니다** — 이슈 044·045의 이미지 항목만 대기 |

**착수를 막는 게이트가 없다.** G3·G6은 특정 이슈만 제한하고 나머지 49개는 진행 가능하다.

### 5.1 개정 이력

| 일자 | 무엇 |
|---|---|
| 2026-08-07 | §4.1 신설 — "1a 화면 명세 완료" 결론 정정 ([G-18](./prd/GAPS.md#g-18)) |
| 2026-08-07 | G7 정정 — 법률 검토가 트리거 기반으로 분해돼 착수 차단이 아님 ([SPEC-04 §8.1](./spec/SPEC-04_비기능요구사항.md)) |
| 2026-08-07 | §3.5 `FR-USER-002` ERD 칸에 `ingredient_brand.is_sponsored` 병기 ([G-19](./prd/GAPS.md#g-19)) |
