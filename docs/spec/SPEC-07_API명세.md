...
> ⚠ **호스팅 제약 (2026-08-21 개정)** — 브라우저가 API를 직접 부른다면 프론트와 API가
> 같은 상위 도메인이어야 한다(`www.example.kr` / `api.example.kr`). 다만 지금 구조는
> 브라우저가 **웹 오리진의 프록시만** 부르므로 쿠키도 웹 오리진에만 있으면 되고,
> 상위 도메인이 같을 필요가 없다 — 판정과 성립 조건은 [G-45](../prd/GAPS.md#g-45).
