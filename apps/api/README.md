...
```
GET  /api/v1/auth/csrf        →  { headerName, token }
POST /api/v1/...              →  X-CSRF-Token: <token>
```

토큰은 **세션에 바인딩**된다. 쿠키 방식이면 토큰이 브라우저에 있어 XSS 로 읽히면 그대로 뚫린다.

**면제는 `/events` 하나뿐이고 코드 상수다** (`CsrfExemptions`). 인증이 필요 없고 부작용이 집계뿐이라
CSRF 대신 레이트 리밋이 방어한다. 설정 파일로 빼지 않는 이유는 늘어나기 때문이다 —
...
| `/search` | 60/분 | IP | 열림 |
| `/events` | 120/분 | **세션** | 열림 |
| `/auth/{provider}/callback` | 10/분 | IP | **닫힘** |
| 어드민 쓰기 | 60/분 | **사용자** | **닫힘** |

**기준이 대상마다 다르다.** 하나로 뭉뚱그리면 `/events` 가 공유 IP 뒤에서 서로를 막고,
어드민 쓰기는 같은 사무실의 두 에디터가 한 통을 나눠 쓴다.

**읽기는 열고 쓰기는 닫는다** (DECISIONS §1). 레이트 리밋 장애로 사이트가 통째로 안 보이는 것이
그 순간의 과도한 트래픽보다 나쁘고, 반대로 쓰기를 열어 두면 그 틈에 벌어진 일을 되돌려야 한다.
...
> **인메모리다.** 인스턴스가 하나라는 전제이고, 늘어나면 통이 인스턴스마다 따로 생겨
> 실효 한도가 N배가 된다. 그때 재검토한다 (SPEC-08 §9).
>
> **IP 는 메모리 키로만 쓴다.** 저장하지도 로그에 남기지도 않는다 (DECISIONS §1 · `PRIN-D04`).

## 저장 시점 정화 (SPEC-08 §7)

에디터 본문은 **저장할 때** 정화한다. 렌더링 시점에만 처리하면 출력 경로마다 놓친다 —
SSG 빌드 · 어드민 미리보기 · OG 태그 생성 · 검색 색인. 경로는 이슈가 늘수록 늘어나고,
새 경로를 만드는 사람은 정화가 필요하다는 것을 모른다.

**저장된 값 자체가 안전해야** 출력 경로가 몇 개든 상관없다.

```kotlin
val body = MarkdownSanitizer.sanitize(request.body)   // 저장 직전
```

마크다운 원문을 다룬다. HTML 로 변환한 뒤 정화하면 저장되는 것이 HTML 이 되고,
에디터가 다시 열었을 때 자기가 쓴 것과 다른 것을 보게 된다.
