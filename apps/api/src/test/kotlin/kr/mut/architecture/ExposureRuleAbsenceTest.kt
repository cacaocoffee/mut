...
import kr.mut.common.security.authz.PermissionMatrix
...
class ExposureRuleAbsenceTest {
        ...
        assertAll(
            {
                ...
                assertThat(configs.map { ourNamespace(Files.readString(it)) })
                    .`as`("mut 블록을 하나도 못 읽으면 이 테스트는 아무것도 지키지 않는다")
    ...
    private fun ourNamespace(yaml: String): String {
        ...
        val start = lines.indexOfFirst { it.startsWith("mut:") }
        if (start < 0) return ""
    ...
    companion object {
        private const val SOURCE_PATH =
            "src/test/kotlin/kr/mut/architecture/ExposureRuleAbsenceTest.kt"
