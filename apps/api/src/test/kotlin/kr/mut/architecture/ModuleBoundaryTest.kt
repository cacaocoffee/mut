 ...
 * 루트가 `kr.mut` 이든 `kr.mut.architecture.fixture` 든 똑같이 걸린다.
 *
 * ## 예외를 등록할 때
 *
...
class ModuleBoundaryTest {
    ...
    fun `모듈 아래 어떤 패키지든 그래프에 보인다`() {
        ...
        assertThat(production().filter { coordinateOf(it.packageName) == null }.map { it.name })
            .`as`("kr.mut 아래인데 좌표가 없는 클래스")
            .allSatisfy { name ->
                assertThat(name).matches("^kr\\.mut\\.[A-Z].*")
            }
    }
    ...
    fun `RED9 - 8개 규칙이 현재 코드베이스에서 통과한다`() {
        ...
        assertThat(classes.map { it.name })
            ...
            .contains("kr.mut.MutApplication")
...
private const val PRODUCTION_ROOT = "kr.mut"
private const val FIXTURE_ROOT = "kr.mut.architecture.fixture"
...
private val ALLOWED_TARGETS: Map<String, Set<String>> = mapOf(
    ...
    // `content` 는 2026-08-28 개정분이다 — 아티클이 DB 로 오며(ADR-0011) 북마크 대상이 됐다.
    // 팬아웃은 여전히 2다(cocktail·content) — stock 은 코드에 없다(내 술장 Phase 2 회귀).
    "user" to setOf("stock", "cocktail", "content"),
    "stock" to emptySet(),
 ...
 * 목록을 늘리는 대신 규칙을 바꾼다.
 *
 * 루트에 상관없이 동작한다 — 프로덕션은 `kr.mut.cocktail.domain`,
 * 픽스처는 `kr.mut.architecture.fixture.cocktail.domain` 이다.
 * `mut` 은 세그먼트 전체가 일치하지 않으므로 `cocktail` 로 오인되지 않는다.
 */
