...
class SocialLoginApiTest {
    ...
    fun `RED16 - 어댑터가 인터페이스 뒤에 있다`() {
        val vendorAware = listOf(
            kr.kcocktail.user.domain.User::class,
            kr.kcocktail.user.oauth.SocialLoginService::class,
    ...
    fun `RED35 - callback 이 10rpm 으로 제한된다`() {
        val policy = kr.kcocktail.common.security.ratelimit.RateLimitPolicy.AUTH_CALLBACK
        ...
        assertAll(
            ...
            { assertThat(policy.window.toMinutes()).isEqualTo(1) },
            {
                assertThat(policy.onStoreFailure)
                    ...
                    .isEqualTo(kr.kcocktail.common.security.ratelimit.FailMode.CLOSED)
            },
            {
                assertThat(kr.kcocktail.common.security.ratelimit.RateLimitRoutes.policyFor(request))
                    .`as`("콜백 경로가 이 정책에 걸린다")
                    .isEqualTo(policy)
            },
        )
    ...
    companion object {
        ...
        private const val STATE_PREFIX = "kcocktail.oauth.state."
        private const val REDIRECT_BASE = "https://api.kcocktail.test"
        private const val ALLOWED_RETURN = "https://www.kcocktail.test"
        ...
        fun props(registry: DynamicPropertyRegistry) {
            ...
            registry.add("kcocktail.oauth.redirect-base") { REDIRECT_BASE }
            registry.add("kcocktail.oauth.allowed-returns[0]") { ALLOWED_RETURN }

            registry.add("kcocktail.oauth.kakao.client-id") { "kakao-test" }
            registry.add("kcocktail.oauth.kakao.client-secret") { "kakao-secret" }
            registry.add("kcocktail.oauth.kakao.authorize-uri") { "$stub/kakao/authorize" }
            registry.add("kcocktail.oauth.kakao.token-uri") { "$stub/kakao/token" }
            registry.add("kcocktail.oauth.kakao.user-info-uri") { "$stub/kakao/me" }
            registry.add("kcocktail.oauth.kakao.scopes[0]") { "profile_nickname" }
            registry.add("kcocktail.oauth.kakao.scopes[1]") { "account_email" }

            registry.add("kcocktail.oauth.naver.client-id") { "naver-test" }
            registry.add("kcocktail.oauth.naver.client-secret") { "naver-secret" }
            registry.add("kcocktail.oauth.naver.authorize-uri") { "$stub/naver/authorize" }
            registry.add("kcocktail.oauth.naver.token-uri") { "$stub/naver/token" }
            registry.add("kcocktail.oauth.naver.user-info-uri") { "$stub/naver/me" }

            registry.add("kcocktail.oauth.apple.client-id") { "kr.kcocktail.test" }
            registry.add("kcocktail.oauth.apple.team-id") { "TEAM123456" }
            registry.add("kcocktail.oauth.apple.key-id") { "KEY1234567" }
            registry.add("kcocktail.oauth.apple.private-key-pem") { SocialProviderStub.clientSecretKeyPem }
            registry.add("kcocktail.oauth.apple.authorize-uri") { "$stub/apple/authorize" }
            registry.add("kcocktail.oauth.apple.token-uri") { "$stub/apple/token" }
            registry.add("kcocktail.oauth.apple.jwks-uri") { "$stub/apple/keys" }
        }
    }
}
