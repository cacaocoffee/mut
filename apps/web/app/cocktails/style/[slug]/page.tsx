...
export async function generateMetadata({
  params,
}: PageProps<"/cocktails/style/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const view = await categoryView(AXIS, slug);
  return view ? categoryMetadata(AXIS, view) : {};
}

export default async function Page({ params }: PageProps<"/cocktails/style/[slug]">) {
  const { slug } = await params;
  const view = await categoryView(AXIS, slug);
  if (!view) notFound();
