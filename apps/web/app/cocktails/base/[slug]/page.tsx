...
export async function generateMetadata({
  params,
}: PageProps<"/cocktails/base/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const view = await categoryView(AXIS, slug);
  return view ? categoryMetadata(AXIS, view) : {};
}

export default async function Page({ params }: PageProps<"/cocktails/base/[slug]">) {
  const { slug } = await params;
  const view = await categoryView(AXIS, slug);
  if (!view) notFound();
